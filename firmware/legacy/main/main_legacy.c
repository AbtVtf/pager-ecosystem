...
typedef enum {
    ...
    // On-device AI agent — the cyberdeck "jack-in" terminal.
    SHELL_PAGE_AGENT,
} shell_page_t;
...
static struct {
    ...
    // --- AGENT (cyberdeck) ------------------------------------------ //
    //
    // The agent runtime owns the transcript + inference task; the shell
    // only owns the input buffer + a status line surfaced via observer
    // events. Drawn full-screen with the lock-screen aesthetic (matrix
    // rain + scanlines) so the "jack in" mood carries over.
    char agent_input[256];               // bound to s.text_buf when AGENT page is up
    char agent_status[80];               // "READY" / "THINKING" / "TOOL wifi_scan" / "ERROR: …"
    int  agent_scroll;                   // 0 = follow tail; +N = lines pinned from top
...
// Render mutex — the cyber_fx animation task and the input router task
// can both request a render; the mutex serialises framebuffer +
// SPI-bus access so we don't race a half-painted frame to the panel.
                           ...
                           pageros_chrome_state_t *cs)
{
    ...
    // Matrix rain backdrop — sits behind tiles, then chrome bars + rail
    // are painted on top.
    pageros_cyber_fx_paint_matrix(canvas, &g_palette, vp_x, vp_y, vp_w, vp_h);
    ...
    // Focus sweep — overlay animated brackets + travelling light on the
    // focused tile rectangle. We replicate the tile_grid layout math.
    if (focus >= 0) {
        int cols = 3, rows = 3;
        int pad = 6;
        int tile_w = (vp_w - pad * (cols + 1)) / cols;
        int tile_h = (vp_h - pad * (rows + 1)) / rows;
        int r = focus / cols, c = focus % cols;
        int tx = vp_x + pad + c * (tile_w + pad);
        int ty = vp_y + pad + r * (tile_h + pad);
        // Periodic glitch slice on the focused tile (seed uses the
        // absolute tile index so paging keeps the same element seed).
        pageros_cyber_fx_paint_element_glitch(canvas, &g_palette,
                                              tx + 2, ty + 2,
                                              tile_w - 4, tile_h - 4,
                                              (uint32_t)s.tile_index * 73u + 11u);
        pageros_cyber_fx_paint_focus(canvas, &g_palette,
                                     tx, ty, tile_w, tile_h);
    }
    ...
    // Soft scanline overlay across the viewport for the CRT vibe.
    pageros_widgets_chrome_scanlines(canvas, vp_x, vp_y, vp_w, vp_h, 3);
}
...
static void render_lock(pageros_fonts_canvas_t *canvas)
{
    ...
    // Matrix rain across the full lock screen.
    pageros_cyber_fx_paint_matrix(canvas, &g_palette, 0, 0,
                                  canvas->w, canvas->h);

    // Top tag — looks like a system header
    ...
    pageros_fonts_draw_text(canvas, canvas->w - 88, 4,
                            "v0.PRE-ALPHA", -1, g_palette.dim, 90);

    // Decorative JP brackets around the clock zone.
    pageros_cyber_fx_draw_bracket_pair(canvas, 50, 24,
                                       canvas->w - 100, 64,
                                       g_palette.accent);

    // Clock huge, accent-magenta, with cyan shadow 1 px offset for the
    ...
    // Identity below clock — with surrounding accent ticks.
    char fp[PAGEROS_IDENTITY_FP_LEN] = {0};
    ...
    // PIN entry — chunky cells with internal scanline tint.
    ...
    if (s.lock_pin_required) {
        ...
        for (int i = 0; i < 4; i++) {
            int cx0 = x0 + i * (32 + 8);
            // Black backdrop so the rain doesn't poke through cells.
            pageros_widgets_fill_rect(canvas, cx0, box_y, 32, 32,
                                      g_palette.bg);
            pageros_widgets_outline_rect(canvas, cx0, box_y, 32, 32,
                                         g_palette.info);
            // Inner accent frame for filled cells.
            if (i < filled) {
            ...
            // Corner ticks
            pageros_widgets_fill_rect(canvas, cx0 - 2, box_y - 2, 4, 1,
                                      g_palette.accent);
            pageros_widgets_fill_rect(canvas, cx0 - 2, box_y - 2, 1, 4,
                                      g_palette.accent);
            pageros_widgets_fill_rect(canvas, cx0 + 30, box_y + 30, 4, 1,
                                      g_palette.accent);
            pageros_widgets_fill_rect(canvas, cx0 + 31, box_y + 30, 1, 4,
                                      g_palette.accent);
        }
    ...
    // Strong scanlines on lock screen — it's a "boot/idle" surface.
    pageros_widgets_chrome_scanlines(canvas, 0, 0, canvas->w, canvas->h, 2);

    // Bottom tag — power/transport status.
    char btag[64];
...
// --- AGENT (cyberdeck) full-screen render ------------------------- //
//
// Same visual frame as render_lock — matrix rain + scanlines + cyber
// tag — but the body is a transcript pane plus an input line at the
// foot. No chrome bars / no sidebar rail: we want the "jacked in"
// immersion every time the user opens this app.

static int agent_role_color(pageros_agent_role_t r)
{
    switch (r) {
    case PAGEROS_AGENT_ROLE_USER:      return g_palette.accent;
    case PAGEROS_AGENT_ROLE_ASSISTANT: return g_palette.info;
    case PAGEROS_AGENT_ROLE_TOOL:      return g_palette.warn;
...
static const char *agent_role_tag(pageros_agent_role_t r)
{
    ...
    case PAGEROS_AGENT_ROLE_ASSISTANT: return "AST>";
...
// Naive char-wrapping. Returns the number of *display lines* the text
// occupied, given a maximum pixel width and the default font metrics
// (~6 px advance). We draw line N starting at y_base + N * line_h, but
// only between y_clip_top and y_clip_bottom (inclusive lower bound).
static int agent_draw_wrapped(const pageros_fonts_canvas_t *canvas,
                              int x, int y_base, int max_w, int line_h,
                              int y_clip_top, int y_clip_bottom,
                              const char *text, uint16_t color)
{
    if (!text || !*text) return 0;
    int per_line = max_w / 6;     // 5x7 glyph + 1px advance
    ...
    while (*p) {
        char buf[120];
        int n = 0;
        // Hard newlines mid-string still get their own row.
        ...
        if (y >= y_clip_top && y + 8 <= y_clip_bottom) {
...
static void render_agent(pageros_fonts_canvas_t *canvas,
                         pageros_chrome_state_t *cs)
{
    (void)cs;  // no chrome bars on this page

    pageros_widgets_fill_rect(canvas, 0, 0, canvas->w, canvas->h, g_palette.bg);
    pageros_cyber_fx_paint_matrix(canvas, &g_palette, 0, 0,
                                  canvas->w, canvas->h);

    // Top tag — mirrors the lock screen's [SYS//…] header.
    char tag[80];
    const char *status = s.agent_status[0] ? s.agent_status : "READY";
    snprintf(tag, sizeof(tag), "[SYS//AGENT·%.60s]", status);
    pageros_fonts_draw_text(canvas, 8, 4, tag, -1,
                            g_palette.info, canvas->w);
    pageros_fonts_draw_text(canvas, canvas->w - 88, 4,
                            "JACK-IN MODE", -1,
                            g_palette.dim, 90);
    ...
    int t_x = 8;
    int t_top = 18;
    int t_bot = canvas->h - 22;        // leave 22px for the input line
    int line_h = 10;
    int max_w = canvas->w - 16;

    // First pass: measure how many lines each visible turn occupies so
    // we can lay them out from the bottom up. Skip system prompt — it's
    // long and not interesting on-screen.
    ...
    // Pre-compute line counts per turn (cheap; n_turns ≤ 32).
    struct { int lines; } counts[64];
    if (n_turns > (int)(sizeof(counts) / sizeof(counts[0]))) {
        n_turns = (int)(sizeof(counts) / sizeof(counts[0]));
    }
    int total_lines = 0;
    for (int i = 0; i < n_turns; i++) {
        pageros_agent_turn_t t = {0};
        if (!pageros_agent_transcript_get(i, &t)) { counts[i].lines = 0; continue; }
        if (t.role == PAGEROS_AGENT_ROLE_SYSTEM) { counts[i].lines = 0; continue; }
        const char *text = t.content ? t.content : (t.tool_summary ? t.tool_summary : "");
        int lines = 1;
        ...
        counts[i].lines = lines + (t.role == PAGEROS_AGENT_ROLE_ASSISTANT && t.tool_summary ? 1 : 0);
        total_lines += counts[i].lines;
    }

    // Find first turn to render so the tail fits in `budget` lines.
    int first = 0;
    int running = total_lines;
    while (first < n_turns && running > budget) {
        running -= counts[first].lines;
        first++;
    }
    ...
    for (int i = first; i < n_turns; i++) {
        ...
        const char *tagp = agent_role_tag(t.role);

        // Role prefix on its own short row.
        pageros_fonts_draw_text(canvas, t_x, y, tagp, -1, color, 40);

        // For assistant turns with a tool_summary, show that as the
        // single-line content (the textual content is usually empty).
        const char *text;
        if (t.role == PAGEROS_AGENT_ROLE_ASSISTANT && t.tool_summary
            ...
            int used = agent_draw_wrapped(canvas, t_x + 36, y, max_w - 36,
                                          line_h, t_top, t_bot,
                                          dispatch, color);
            y += line_h * (used > 0 ? used : 1);
            continue;
        }
        text = t.content ? t.content
                         : (t.tool_summary ? t.tool_summary : "(empty)");

        int used = agent_draw_wrapped(canvas, t_x + 36, y, max_w - 36,
                                      line_h, t_top, t_bot, text, color);
        y += line_h * (used > 0 ? used : 1);
    }

    // Footer separator + input line.
    pageros_widgets_fill_rect(canvas, 0, t_bot, canvas->w, 1, g_palette.dim);

    // Prompt + buffered input. Append a "_" cursor to make it feel alive.
    char prompt_line[280];
    bool ready = pageros_agent_is_ready();
    if (!ready) {
        snprintf(prompt_line, sizeof(prompt_line),
                 "[OFFLINE]  no /sd/agent/config.json");
        pageros_fonts_draw_text(canvas, 8, t_bot + 6, prompt_line, -1,
                                g_palette.error, canvas->w);
    } else if (pageros_agent_is_busy()) {
        snprintf(prompt_line, sizeof(prompt_line),
                 "[%s]  please wait…", status);
        pageros_fonts_draw_text(canvas, 8, t_bot + 6, prompt_line, -1,
                                g_palette.warn, canvas->w);
    } else {
        const char *cursor = ((esp_timer_get_time() / 500000) & 1) ? "_" : " ";
        snprintf(prompt_line, sizeof(prompt_line),
                 ">_ %s%s", s.agent_input, cursor);
        pageros_fonts_draw_text(canvas, 8, t_bot + 6, prompt_line, -1,
                                g_palette.accent, canvas->w);
    }

    // Scanlines overlay for the jack-in feel.
    pageros_widgets_chrome_scanlines(canvas, 0, 0, canvas->w, canvas->h, 2);
}
...
static void render_screen_locked(void)
{
    ...
    case SHELL_PAGE_AGENT:        render_agent(&canvas, &cs); break;
    }
    // Page-transition scanwipe + invert strips — sits above content but
    // under the toast so error banners remain readable mid-wipe.
    pageros_cyber_fx_paint_transition(&canvas, &g_palette);
    // Toast banner floats above everything.
    pageros_widgets_chrome_toast(&canvas, &g_palette);
    // Final pass — CRT glitch slices, action ripple, edge vignette.
...
static void render_screen(void)
{
    // Auto page-transition detection — any time s.page changes between
    // two render calls we kick a scanwipe + glitch via cyber_fx so the
    // user sees a clear hand-off.
    static int last_page = -1;
    if ((int)s.page != last_page) {
        last_page = (int)s.page;
...
// Agent observer — fires from the agent inference task. We update the
// shell-side status label (rendered in the top tag of the AGENT page)
// and request a fresh frame. render_screen() takes g_render_mu, so the
// cross-task call is safe.
...
static void mount_agent(void)
{
    ...
    s.text_buf = s.agent_input;
    s.text_cap = sizeof(s.agent_input);
...
static void switch_to_builtin(const char *id)
{
    ...
    else if (strcmp(id, "builtin:agent")  == 0) mount_agent();
...
static bool default_shell_handler(const pageros_router_event_t *ev, void *ctx)
{
    ...
    // Every ENTER activation gets a small action ripple from cyber_fx,
    // and a glitch nudge — the UI feels like it's *responding*.
    if (ev->kind == PAGEROS_ROUTER_EVT_NAV
            && ev->as.nav == PAGEROS_ROUTER_NAV_ENTER) {
        pageros_cyber_fx_on_action(PAGEROS_DISPLAY_WIDTH / 2,
                                   PAGEROS_DISPLAY_HEIGHT / 2);
        pageros_cyber_fx_kick_glitch();
    }
    ...
    case PAGEROS_ROUTER_EVT_NAV: {
        ...
        if (ev->as.nav == PAGEROS_ROUTER_NAV_ENTER) {
            ...
            if (s.page == SHELL_PAGE_AGENT) {
                ...
                if (!pageros_agent_is_ready()) {
                    pageros_toast("AGENT OFFLINE — /sd/agent/config.json",
                                  PAGEROS_TOAST_ERROR, 2500);
                    return true;
                }
                if (pageros_agent_is_busy()) return true;
                esp_err_t r = pageros_agent_submit(s.agent_input);
                if (r == ESP_OK) {
                    s.agent_input[0] = '\0';
                    ...
                    pageros_toast("AGENT SUBMIT FAILED",
        ...
        if (ev->as.nav == PAGEROS_ROUTER_NAV_BACK) {
            // AGENT page: BACK on empty buffer → exit. Non-empty → backspace.
            if (s.page == SHELL_PAGE_AGENT) {
    ...
    case PAGEROS_ROUTER_EVT_KEY: {
        ...
        if (ch == '\n') {
            ...
            else if (s.page == SHELL_PAGE_AGENT) {
                if (s.agent_input[0] != '\0'
                        && pageros_agent_is_ready()
                        && !pageros_agent_is_busy()) {
...
static void boot_log_add(boot_log_level_t lvl, const char *fmt, ...)
{
    ...
    g_boot_log.n++;
    // Kick a glitch on every line so the splash strobes during boot.
    pageros_cyber_fx_kick_glitch();
}
...
static void boot_splash_render(void)
{
    ...
    // Matrix rain across the whole screen — boot has no chrome bars to
    // protect, the rain is the chrome.
    pageros_cyber_fx_paint_matrix(&canvas, &g_palette,
                                  0, 0, canvas.w, canvas.h);

    // Decorative JP brackets around the header zone.
    pageros_cyber_fx_draw_bracket_pair(&canvas, 16, 4,
                                       canvas.w - 32, 56,
                                       g_palette.accent);

    // Header — "PAGEROS" at 2x scale with chroma split.
    const char *hdr = "PAGEROS";
    int hdr_w = 16 * 2 * (int)strlen(hdr);
    int hdr_x = (canvas.w - hdr_w) / 2;
    pageros_fonts_draw_text_scaled_16(&canvas, hdr_x + 2, 14, hdr, -1,
                                      g_palette.accent, 2);
    pageros_fonts_draw_text_scaled_16(&canvas, hdr_x, 12, hdr, -1,
                                      g_palette.info, 2);
    // Magenta accent dot to the right of the header.
    pageros_widgets_fill_rect(&canvas, hdr_x + hdr_w + 6, 22, 8, 8,
                              g_palette.accent);

    // Subtitle with shifting chars
    const char *sub = "// CYBERDECK OS · v0.PRE-ALPHA · 2087";
    int sub_w = pageros_fonts_measure_text(sub, -1);
    pageros_fonts_draw_text(&canvas, (canvas.w - sub_w) / 2, 48,
                            sub, -1, g_palette.info, canvas.w);
    ...
    if (pageros_identity_fingerprint(fp) == ESP_OK && fp[0]) {
        ...
        pageros_fonts_draw_text(&canvas, (canvas.w - bw) / 2, 62,
                                buf, -1, g_palette.dim, canvas.w);
    }

    // Divider — accent + cyan double rule for the cyberpunk flair.
    pageros_widgets_fill_rect(&canvas, 24, 74, canvas.w - 48, 1, g_palette.accent);
    pageros_widgets_fill_rect(&canvas, 24, 76, canvas.w - 48, 1, g_palette.info);

    // Log lines — render each with a small leading tick char so they
    // look like a console scroll.
    int y = 82;
    int line_h = 11;
    for (int i = 0; i < g_boot_log.n; i++) {
        ...
        // Black plate behind text so it reads over the rain.
        int tw = pageros_fonts_measure_text(g_boot_log.text[i], -1);
        pageros_widgets_fill_rect(&canvas, 22, y - 1, tw + 16, 10,
                                  g_palette.bg);
        // Leading arrow
        pageros_fonts_draw_text(&canvas, 24, y, "▶", -1,
                                g_palette.accent, 12);
        pageros_fonts_draw_text(&canvas, 32, y,
    ...
    // Scanlines + global glitch overlay
    pageros_widgets_chrome_scanlines(&canvas, 0, 0, canvas.w, canvas.h, 2);
    pageros_cyber_fx_paint_overlay(&canvas, &g_palette);
...
void app_main(void)
{
    ...
    // Cyber FX — animation engine + matrix rain. Render mutex must exist
    // before the fx task starts so the task can lock the framebuffer.
    g_render_mu = xSemaphoreCreateMutex();
    pageros_cyber_fx_init();
    boot_log_add(BL_OK, "[OK] CYBER FX ENGINE (48 KATAKANA)");
    boot_splash_render();
    ...
    // Agent harness — loads /sd/agent/config.json, registers tools,
    // spawns the inference task. Soft-fails if the config is missing;
    // the AGENT app surfaces the OFFLINE state to the user.
    esp_err_t agent_err = pageros_agent_init();
    if (agent_err != ESP_OK) {
        ESP_LOGW(TAG, "agent init skipped: %s", esp_err_to_name(agent_err));
    } else {
        boot_log_add(BL_OK, "[OK] AGENT HARNESS");
    }

    // Cyber FX background task: paints matrix rain + glitch overlay at
    // ~25 fps. Uses g_render_mu so its calls to render_screen() don't
    // race the input-driven render path on the framebuffer / SPI bus.
